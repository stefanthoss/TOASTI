...
<?php echo if($this->Permissions->check('Users.delete')) { $this->Form->postLink('<i class="icon-trash icon-white"></i> Nutzer löschen', array('action' => 'delete', $user['User']['id']), array('class' => 'btn btn-danger', 'escape' => false), 'Bist du dir sicher, dass du den Nutzer '.$user['User']['username'].' löschen willst? Das kann nicht rückgängig gemacht werden.'); } ?>
